...
The exercise with localeStorage where we had to serialize data is interesting. It's not useful in our case now, but if there was a dependancy on type of data coming out of the localeStorage then maybe it'd be useful.

Interesting concept of handling a function to useState line 17 in case we just need to calculate expensive stuff for the deault value. That way, the function will be called once instead of having it in the components body and being re-called multiple times on re-rendering
