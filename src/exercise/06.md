...
This is applicable for exercise 1 ->
sequence of renders:
 1. when we submit the form with a new name PokemonInfo renders:
 - inside component body (React renders UI first and then go to side effects)
 - render loading
 - inside use effect
 - inside component body
 - render pokemondata

2. when we submit the form with another pokemon name while we fetched: one before
- inside component body
- inside component body
- render pokemondata
- inside use effect
...
Whenever we fetch some data form exertnal API and we need to change the UI based on the various conditions involving the success or failure of getting that data, it's better to use a status state to update it whenever it's needed. This way we don't need to reset other states which our conditions might be dependant on and also we make those conditions much clearer.

When multiple states are used that are dependant on each other, consider having an object of state with all states as properties.

One thing you can do when you want to reset the state of a component, is by providing it a key prop which can be used to unmount and re-mount a component (the example with ErrorBoundary).
